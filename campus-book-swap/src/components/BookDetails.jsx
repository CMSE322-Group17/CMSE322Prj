                ...
                {book.bookType && actions[book.bookType] && (
                  <>
                    <button 
                      ...
                      className={`w-full py-2 px-4 ${isSold ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'} text-white rounded-lg transition-colors`}
                      disabled={isSold}
                    >
                      {isSold ? 'Sold' : actions[book.bookType].primary}
                    </button>
                    <button 
                      onClick={() => handleActionClick('secondary')}
                      className={`w-full py-2 px-4 ${isSold ? 'bg-gray-300 cursor-not-allowed' : 'bg-gray-100 hover:bg-gray-200'} text-gray-800 rounded-lg transition-colors`}
                      disabled={isSold}
                    >
                      {isSold ? 'Unavailable' : actions[book.bookType].secondary}
                    </button>
                  </>
                )}
                <button 
                  onClick={() => !isSold && navigate(`/chat/${seller.id}/${book.id}`)}
                  className={`w-full py-2 px-4 border border-gray-300 text-gray-600 rounded-lg ${isSold ? 'bg-gray-200 cursor-not-allowed' : 'hover:bg-gray-50'} transition-colors flex items-center justify-center gap-2`}
                  disabled={isSold}
                >
